...
///
/// A `.ready` marker stores the installed app package version, allowing an
/// existing extraction to be reused while still refreshing model files after
/// the APK is updated. The cache key is the base64 of the asset directory name
/// to avoid collisions on the disk.
Future<String> _extractAssetDirToCache(String assetDir) async {
  ...
  final cacheVersion =
      await FlutterLive2dPlatform.instance.getAssetCacheVersion();
  ...
  if (outDir.existsSync() && marker.existsSync()) {
    final extractedVersion = await marker.readAsString();
    if (extractedVersion == cacheVersion) {
  ...
  await marker.writeAsString(cacheVersion, flush: true);
  return _ensureTrailingSlash(outDir.path);
}
